...
func (db *Database) IncreaseUserBalance(sqlTx *sql.Tx, address string, amount uint64) error {
	query := `
	...
	if _, err := sqlTx.Exec(query, address, amount); err != nil {
		return err
	}
...
func (db *Database) DecreaseUserBalance(sqlTx *sql.Tx, address string, amount uint64) error {
	// First, check if balance is sufficient
	var currentBalance uint64
	checkQuery := `SELECT balance FROM balances WHERE address = ?`

	if err := sqlTx.QueryRow(checkQuery, address).Scan(&currentBalance); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	_, err := sqlTx.Exec(updateQuery, amount, address)
